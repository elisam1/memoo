...
function initAdmin() {
  ...
  if (!svcJson && !svcFile && !svcBase64) {
    console.warn('Firebase Admin service account not set (FIREBASE_SERVICE_ACCOUNT_JSON, FIREBASE_SERVICE_ACCOUNT_FILE/GOOGLE_APPLICATION_CREDENTIALS, or FIREBASE_SERVICE_ACCOUNT_BASE64). Token verification disabled.');
    initialized = true;
    return admin;
  }
  try {
...
module.exports = {
  ...
  getStatus: function getStatus() {
    initAdmin();
    const initializedApps = admin.apps.length;
    ...
    let projectId = null;
    try {
      projectId = initializedApps ? (admin.app().options && admin.app().options.projectId) : null;
    } catch (_) {
      projectId = null;
    }
    projectId = projectId || process.env.FIREBASE_PROJECT_ID || null;
    return {
      initialized: !!initializedApps,
      configSource: source,
      projectId,
    };
  }
