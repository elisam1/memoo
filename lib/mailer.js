...
async function sendReplyReceived({ memo, reply, baseUrl }) {
  const subject = `Reply on Memo: ${memo.title}`;
  const html = await render('reply_received', {
    memo,
    reply,
    baseUrl,
  ...
  const from = process.env.MAIL_FROM || 'Memoo <no-reply@memoo.local>';
  const to = memo.hrEmail || (process.env.FALLBACK_HR_EMAIL || '');
  if (!to) return; // no HR email configured
  await transporter.sendMail({ from, to, subject, html });
}

module.exports = { sendMemoCreated, sendReplyReceived };