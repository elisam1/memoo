...
async function sendMemoCreated({ memo, baseUrl, companyConfig }) {
  ...
  const mailFrom = process.env.MAIL_FROM || companyConfig.auth.user;

  const mailOptions = {
    // Use configured sender for deliverability; set replyTo to HR
    from: mailFrom,
    replyTo: memo.hrEmail,
    to: memo.managerEmail,
    subject: `[Memoo] New Memo: ${memo.title}`,
    html: `
      <p>Hello,</p>
  ...
  console.log(`New memo email sent to ${memo.managerEmail}`);
}
...
async function sendReplyNotification({ memo, reply, baseUrl, companyConfig }) {
  ...
  const mailOptions = {
    // Use configured sender; set replyTo to actual sender
    from: mailFrom,
    replyTo: reply.email,
    to: recipient,
    subject: `[Memoo] New Reply on Memo: ${memo.title}`,
    html: `
      <p>Hello,</p>
  ...
  await transporter.sendMail(mailOptions);
  console.log(`Reply notification sent to ${recipient}`);
}
